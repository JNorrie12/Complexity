...
if input is 'a':
    ...
    red_line = mlines.Line2D([], [], color='red', label='L=128')
    purple_line = mlines.Line2D([], [], color='purple', label='L=256')
    black_line = mlines.Line2D([], [], color='black', linestyle='--', label='$\\frac{Heights}{L}=$1.715')
...
#2b)
    ...
    plt.annotate( '$F=k\\sqrt{{\\frac{T}{L^2}}}$', color='red', xy=(0.05, 1.3),xytext=(0.05, 1.3), rotation= 45)
...
if input is 'c':
    ...
    blue_line = mlines.Line2D([], [], color='blue', label='$a_0=$' + str(round(a00,3)))
    orange_line = mlines.Line2D([], [], color='orange', label='$a_0=$' + str(round(a01,3)) + '(Too small)')
    green_line = mlines.Line2D([], [], color='green', label='$a_0=$' + str(round(a02, 3)) + '(Too large)')
    plt.legend(handles=[blue_line, orange_line, green_line])
    plt.title('Approximation of $a_0$')
    plt.xlabel('$Log(L)$')

    plt.ylabel('$Log(1- \\frac{<h>}{{a_0}\!L})$')
    plt.show()
    LL = np.array(L)
    w= fit[0]
    ...
    grey_line = mlines.Line2D([], [], color='grey', linestyle='--', label='Limit of $\\frac{<h>}{L}$ as $L\\rightarrow \\infty$ $(a_0)$')
    plt.legend(handles=[red_line, grey_line, s])
    plt.title('Fit of Scale Correction, with $a_0$='+str(round(a00,3))+', $a_1$='+str(round(np.exp(fit[1]), 3))+', $\\omega_1=$'+str(round(-w,3)) )
    plt.xlabel('System size L')
    plt.ylabel('$\\frac{<h>}{L}$')
    plt.show()
    ...
    red_line = mlines.Line2D([], [], color='red', label='Scale Correction')
    grey_line = mlines.Line2D([], [], color='gray', linestyle='--', label='Limit of $<h>$ as $L\\rightarrow \\infty$')
    plt.legend(handles=[red_line, grey_line, s])
    plt.show()
elif input is 'z':
    n=3
    ...
    plt.title(' Standard Deviation of Height($\\sigma$) vs. System Size')
    plt.xlabel('System size L')
    plt.ylabel('$\\sigma$')
    ...
    alpha = mlines.Line2D([], [], linestyle=' ', label='Estimated $\\alpha$= ' + str(round(fit[0], 2)))
    b = mlines.Line2D([], [], linestyle=' ', label='Estimated $b_0$=' +str(round(np.exp(fit[1]), 2)))


    plt.legend(handles=[blue_line, a, alpha, b])
    plt.show()
    print fit[0]
    ...
    plt.xlabel('System size $L$')
    plt.ylabel('$\\sigma/L^\\alpha$')
    x = a*np.ones(len(L))
    plt.plot(L, x, color='gray', linestyle=':' )
    black_line = mlines.Line2D([], [], color='black', linestyle=' ', marker='o', label='Data points')
    grey_line= mlines.Line2D([], [], color='gray', linestyle=':', label='Approximate asymptote of $\\frac{\\sigma}{L^\\alpha}$ ('+str(a)+')')
    alpha =mlines.Line2D([], [], linestyle=' ', label='$\\alpha=$' + str(b))
    plt.legend(handles=[black_line, grey_line, alpha])
...
#2d)
    ...
    purple_line = mlines.Line2D([], [], color='purple', label='L=128')
    brown_line = mlines.Line2D([], [], color='brown', label='L=256')
    n =mlines.Line2D([], [], color='red', linestyle='--', label='pdf of $\\mathcal{N(1,0)}$ distribution')
    ...
    ax2.set_ylabel('$\\sigma*P(h;L)$')
    ax2.set_xlabel('Transformed Total Height($\\frac{h-<h>}{\\sigma})$')
    plt.show()